...
- `crackmapexec` - Enumerating Password policy information
- `enum4linux/enum4linux-ng` - Built around `nmblookup`, `net`, `rpcclient` and `smbclient` to use for enumeration of windows hosts and domains.
- `rpcclient` - Enumerating the password policy from SMB Null Sessions or LDAP anonymous bind
- `ldapsearch` - Enumerating the Password Policy from Linux - LDAP Anonymous bind
- `PowerView` - General Enumeration
- `SharpMapExec`, `SharpView`
- 
...
```shell-session
gdxqpardo@htb[/htb]$ cat ilfreight.json 
...
## Enumerating a NULL Session - From Windows
It's less common to do this type of null session attack from windows, but we could use the command `net use \\host\ipc$ "" /u:""` to establish a null session from a windows machine and confirm if we can perform more of this type of attack
...
The choice of tools depends on the goal of the assessment, stealth considerations, any anti-virus or EDR in place, and other potential restrictions on the target host. Let's cover a few examples.
