...
- [create database in memory](#create\database\in\memory)
- [db = sqlite3.connect(':memory:')](#db\=\sqlite3.connect(':memory:'))
- [create database into directory](#create\database\into\directory)
- [get a cursor object](#get\a\cursor\object)
  - [Creating (```CREATE```) and Deleting (```DROP```) Tables](#Creating\(```CREATE```)\and\Deleting\(```DROP```)\Tables)
- [DROP TABLE](#drop\table)
- [CREATE TABLE](#create\table)
  - [Inserting (```INSERT```) Data into the Database](#Inserting\(```INSERT```)\Data\into\the\Database)
- [use list of users for inserting multiple user info](#use\list\of\users\for\inserting\multiple\user\info)
...
- [update user phone with id = 5](#update\user\phone\with\id\=\5)
- [delete user row with id = 8](#delete\user\row\with\id\=\8)
...
- [A very secure password](#a\very\secure\password)
  - [SQLite Row Factory and Data Types](#SQLite\Row\Factory\and\Data\Types)
- [close database connection](#close\database\connection)

![[Disclaimer: Contents copied and reformatted (nearly) directly from: ]]
